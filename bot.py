...
async def start(update: Update, context: CallbackContext) -> int:
    ...
    if user_id not in user_data:
        ...
        await message_to_reply.reply_text(
            ...
            'Если вы хотите сделать пост для телеграм канала, вк группы или X, то для начала, пожалуйста, отправьте мне текст, который будет служить подписью к вашей будущей записи в телеграм посте. Текст перенесётся в пост в том форматировании в котором вы его отправите \n\nЕсли текста нет, то напишите "нет".\n\nЛибо воспользуйтесь одной из кнопок ниже для перехода в иные режимы работы:\n\n',                       
...
async def fast_rec(update, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await loading_message.edit_text("ВНИМАНИЕ!!!Если вы хотели сделать пост а не распознать содержимое на изображении, то вернитесь в меню и действуйте в соответвии с инструкциями\n\n✅ Изображение успешно загружено. Выберите действие:", reply_markup=reply_markup)
