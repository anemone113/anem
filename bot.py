...
async def mainhelp_callback(update: Update, context: CallbackContext):
    ...
    Позволяет распознать текст, гриб или растение по их фотографии. Также с помощью встроенной в функцию нейросети можно сделать различные манипуляции с распознанным текстом или узнать, что с растением (болезни, вредители). Кроме того почитать информацию о распознанном растении или получить советы по уходу за ним.  
...
ссылка_1 • ссылка_2</pre>
    ...
    ▶️Кроме того, бот поддерживает загрузку GIF-файлов. Для этого переименуйте .GIF в .RAR, затем отправьте файл боту во время оформления поста. Это нужно для того, чтобы телеграм не пережимал GIF-файлы как он это делает по-сути превращая их в mp4. Бот автоматически переименует файл обратно в GIF перед размещением в Telegraph. Внимание! Пост с гифкой в боте будет отображаться в сжатом шакальном виде. Это нормально и нужно для ускорения работы бота. Однако после публикации этого поста в ваш канал GIF опубликуется именно в полноценном полном разрешении. Максимальный размер gif - 20мб 
...
async def run_gpt(update: Update, context: CallbackContext) -> int:
    ...
    await message_to_reply.reply_text("Режим общения с GPT активирован. Отправьте сообщение чтобы начать диалог с ботом или воспользуйтесь одной из команд. Кроме того бот может искать и анализировать информацию в интеренете, содержимое ссылки веб-страницы, распознавать аудио и музыку и тд. Для подробностей воспользуйтесь кнопкой \"помощь\"",  
...
async def handle_role_select(update: Update, context: CallbackContext):
    ...
    if not roles:
        ...
        keyboard = InlineKeyboardMarkup(grouped_default_buttons + [new_role_button])
        message_text = "У вас пока нет своих ролей. Выберите одну из доступных ролей по умолчанию."

        # Отправляем сообщение
        if update.callback_query:
            await update.callback_query.answer()
            await update.callback_query.message.edit_text(message_text, reply_markup=keyboard, parse_mode='Markdown')
        else:
    ...
    keyboard = InlineKeyboardMarkup(grouped_default_buttons + grouped_custom_buttons + [new_role_button])
