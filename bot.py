...
async def run_gpt(update: Update, context: CallbackContext) -> int:
    ...
    await message_to_reply.reply_text("Режим общения с GPT активирован. Отправьте сообщение чтобы начать диалог с ботом или воспользуйтесь одной из команд.\n\n Кроме того если вы начнёте ваше сообщение со слова \"Нарисуй: \" и ваш запрос после двоеточия, то бот нарисует ваш запрос. Если отправите изображение с подписью которая начинается с \"Дорисуй: \" то попытается дорисовать ваш набросок. Для подробностей воспользуйтесь кнопкой \"краткая помощь\"",  
