...
async def start(update: Update, context: CallbackContext) -> int:
    ...
    if user_id not in user_data:
        ...
        await message_to_reply.reply_text(
            ...
            '<i>Так же вы можете воспользоваться одной из кнопок ниже чтобы найти автора по изображению, найти серию и таймметку аниме по кадру из него, проверить вероятность использования ИИ для создания изображения, распознать текст или растение. либо поговорить с ботом. В режиме диалога доступна генерация, дорисовка и редактирвоание изображений.</i>\n\n',
...
async def run_gpt(update: Update, context: CallbackContext) -> int:
    ...
    await message_to_reply.reply_text("Режим общения с GPT активирован. Отправьте сообщение чтобы начать диалог с ботом или воспользуйтесь одной из команд. Так же сли вы начнёте ваше сообщение со слова \"Нарисуй: \" и двоеточия, то бот нарисует ваш запрос. Если отправите изображение с подписью которая начинается с \"Дорисуй: \" то попытается дорисовать ваш набросок. Для подробностей воспользуйтесь кнопкой \"помощь\"",  
...
async def handle_short_gpt_help(update: Update, context: CallbackContext) -> None:
    ...
    help_text_1 = (
        ...
        "Во время диалога с GPT просто напишите **'Нарисуй: *'** и далее, вместо '*', "
        "напишите то, что именно вам нужно.\n\n"
        "Либо отправьте боту изображение с подписью, начинающейся с **'Дорисуй: *'**, где вместо '*' опишите "
        "ваше изображение и укажите, что именно вы желаете видеть на конечном изображении, "
        ...
        "`AnimagineXLV3` - заточена под аниме стилистику. Даёт красочные насыщенные изображения \n"
...
async def handle_gpt_help(update: Update, context: CallbackContext) -> None:
    ...
    help_text_1 = (
        ...
        "однако слишком высокие значения будут наоборот давать непредсказуемые результаты. Кроме того малые значения, около 3-5, дают куда как более художественные результаты чем высокие\n"
        ...
        "Так же доступен ввод значения `seed`. Если описывать простым языком то `seed` - это идентификатор каждого конкретного сгенерированного изображения. Если вам понравилась какая-то из генераций, но вы хотите посмотреть как бы она выглядела с другими настройками, то вы можете использовать её seed для того чтобы изменять конкретно данную генерацию. "
        "Значения seed видны в подписи к каждому изображению в боте, чтобы использовать их, введите данное число сразу после запроса \"Нарисуй:\" или \"Дорисуй:\" и поставьте запятую после числа. Например:\n `Нарисуй: 4357763, дом на дереве`. Если идентификатор не задан в запросе, то каждый раз он генерируется случайным образом\n\n"        
        "**Примеры:**\n"
        "``` Нарисуй:345080. A cat sits on a windowsill, gazing out at a winter scene. Cozy atmosphere, warm light inside, soft snowfall outside. Oil painting (1:2, 9)```\n"
        "Будет сгенерирован кот на подоконнике в соотношении сторон 1:2 и параметром cfg scale равным 9. Идентификатор изображения задан и равен 345080. Если менять параметры но не менять идентификатор, то будет генерироваться примерно одно и то же изображение но выглядящее немного иначе в зависимости от настроек. Если изменить или убрать идентификатор, то будет сгенерировано полностью новое изображение.\n\n"
        ...
        "То же самое, но с параметрами по умолчанию: соотношение 1:1 и cfg scale 3. Идентификатор изображения так же не задан и будет сгенерирован случайно\n\n"
    ...
    help_text_2 = (
        "**1)Отправьте боту одновременно два изображения группой** Первое - то которое вы редактируете, второе - то же самое изображение но в виде маски. Маска представляет из себя двухцветное изображение на котором **чёрный цвет** - те зоны которые НЕ будут меняться, **белые цвет** - напротив зоны на которых будут происходить все дальнейшие изменения. Второе изображение должно быть именно двухцветным, содержать только белый и чёрный цвет без оттенков\n\n"
        "**2)В подписи к этим двум изображениям** напишите \"Замени: *\", где вместо * сформулируйте свой запрос. Например что именно нужно сгенерировать в области редактирования\n\n"
        "**3)Затем, после \"Замени: *\" и ваших указаний** вы можете, по желанию, задать настройки. Это наиболее сложная функция из всех трёх а потому настроек достаточно много. Ниже будет их подробное описание\n\n"           
        "Настройки пишутся в скобках и имеют следующий вид:\n"
        "```  (config:\n"
        "inpainting_full_res=да/нет/true/false\n"
        "cfg_scale=от 1 до 30\n"
        "mask_blur=от 0 до 50\n"
        "inpainting_fill=0, 1, 2 или 3\n"
        "denoising_strength=от 0 до 2)\n"
        "seed=от 0 до 4000000000)\n"        
        "steps=от 5до 50```\n"
        "`inpainting_full_res` - отвечает за то будет ли при генерации учитываться всё изображение целиком или же только область масок, редко когда существенно влияет на конечный результат \n"
        ...
        "**0 - fill** - заполняет область маски соседними цветами. Может быть полезно при удалении объектов с изображения\n"
        ...
        "`denoising_strength` - работает так же как и в предыдущих примерах, но для работы с масками всегда требуются высокие значения, от 0.6 и выше, часто в районе 0.9 или даже ещё выше. \n"
        "`seed` - идентификатор генерации\n"
        "`steps` - Количество \"шагов\" генерации. Значения ниже 15 не рекомендуются, выше 35 не имют смысла потому что особо ничего не дают, но замедляют обработку \n\n"

        "Могут быть указаны как все доступные параметры, так и только часть из них, в любой последовательности, либо быть не указаны вовсе. В таком случае будут применены настройки по умолчанию для отдельных или всех параметров \n"
        "**Пример запроса:**\n"
        "Допустим мы отправляем боту две картинки, первую с пустым креслом около окна. На второй, двухцветной, белым цветом выделена облась в которую мы хоти поместить кота. В таком случае запрос может быть следующим:\n"        
        ...
        "В данном примере будет использоваться всё изображение целиком, inpainting_fill в режиме original учитывает оригинальное изображение под маской, denoising_strength равно 0.82, mask_blur и steps не указана и потому будут использоваться параметры по умолчанию. Seed так же не указан и будет сгенерирован случайно\n\n"      
        "**Ещё один пример:**\n"
        ...
        "```  Замени: удали объект\n```"        
        "В данном примере поле \" (Config: ...)\" не указано вовсе и потому все настройки будут использоваться по умолчанию. Вместо `удали объект` может потребоваться написать что именно должно быть вместо удаляемого объекта.\n\n"    
    )
...
async def gpt_running(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.media_group_id:
        ...
        try:
            ...
            user_prompt = await extract_prompt(caption, user_id)
            ...
            default_settings = {
                ...
                "seed": random.randint(1, 4000000000),  # Генерация случайного seed, если не указан
                "steps": 25  # Стандартное значение для steps
            }

            caption = first_message.caption or ""
            user_settings = parse_settings(caption, default_settings)
            logging.info(f"параметры: {user_settings}")
            ...
            context.user_data['last_image_params'] = {
                "prompt": user_prompt,
                "settings": user_settings,
                "imageData": first_image_base64,
                "maskData": second_image_base64,
            ...
            if job_id:
                ...
                # Проверяем наличие style_preset
                    model_name_map = {
                        ...
                        "sd_xl_base_1.0_inpainting_0.1.safetensors [5679a81a]": "SD_xl_1.0_inpainting"
                    }
                    model_name = model_name_map.get(payload["model"], payload["model"])                                        
                    caption = (
                        f"Изображение ID{payload['seed']} доработано с помощью следующих параметров:\n"
                        f"     Модель: `\"{model_name}\",`\n"
                        f"     Высота: {new_height},\n"                
                        f"     Ширина: {new_width},\n"                
                        f"     Учитывает изображение целиком: `\"{payload['inpainting_full_res']}\",`\n"
                        f"     inpainting fill: `\"{inpainting_fill_descriptions.get(payload['inpainting_fill'], 'Неизвестно')}\",`\n\n"                                         
                        f"`Замени: {user_prompt}\n"
                        f"(config:\n" 
                        f"     inpainting_full_res: {payload['inpainting_full_res']}\n"                        
                        f"     cfg_scale: {payload['cfg_scale']}\n"
                        f"     denoising_strength: {payload['denoising_strength']}\n"
                        f"     inpainting_fill: {payload['inpainting_fill']}\n"                
                        f"     mask_blur: {payload['mask_blur']}\n" 
                        f"     Seed: {payload['seed']}\n"
                        f"     Steps: {payload['steps']})`\n\n"                               
                    )
                    escaped_caption = escape_gpt_markdown_v2(caption)
                    # Удаляем экранирование символа "_" в конце строки
                    escaped_caption = re.sub(r'\\_$', '_', escaped_caption)                                       
                    await update.message.reply_photo(photo=image_url, caption=escaped_caption, reply_markup=generated_img2img_buttons, parse_mode="MarkdownV2")
                else:
    ...
    if update.message.text and update.message.text.lower().startswith("нарисуй:"):
        user_message = update.message.text
        # Извлекаем seed и запрос
        seed_match = re.match(r"нарисуй:\s*(\d+),\s*(.*)", user_message, re.IGNORECASE)
        
        if seed_match:
            seed = int(seed_match.group(1))  # Извлекаем seed
            prompt = seed_match.group(2).strip()  # Извлекаем запрос
        else:
            seed = random.randint(1, 4000000000)  # Генерируем случайное число, если seed не указан
            prompt = user_message[8:].strip()  # Извлекаем запрос без "нарисуй:"

        # Переводим на английский, если нужно


        # Ищем значение steps в квадратных скобках в конце текста
        steps_match = re.search(r"\[(\d+)\]$", prompt)
        if steps_match:
            steps = int(steps_match.group(1))  # Извлекаем значение steps
            prompt = re.sub(r"\[\d+\]$", "", prompt).strip()  # Удаляем [steps] из запроса
        else:
            steps = 30  # Значение по умолчанию
        ...
        prompt = await translate_to_english_if_needed(prompt, update.effective_user.id)
        job_id, payload = await def_prodia(prompt, width, height, cfg_scale, user_id, seed, steps)  # Передаем seed в запрос
        context.user_data['last_image_params'] = {
            "prompt": prompt,
            "width": width,
            "height": height,
            "cfg_scale": cfg_scale,
            "steps": steps,  # Используем значение из [steps] или значение по умолчанию
            "seed": seed
        }
        if job_id:
            image_url = await check_prodia_status(job_id)
    ...
    if update.message.photo:
        try:
            ...
            if user_message.lower().startswith("дорисуй:"):
                ...
                # Проверяем seed и извлекаем запрос
                seed_match = re.match(r"дорисуй:\s*(\d+),\s*(.*)", user_message, re.IGNORECASE)
                if seed_match:
                    seed = int(seed_match.group(1))  # Извлекаем seed
                    user_prompt = seed_match.group(2).strip()  # Извлекаем запрос
                else:
                    seed = random.randint(1, 4000000000)  # Генерируем случайное число, если seed не указан
                    user_prompt = user_message[8:].strip()  # Извлекаем запрос без "дорисуй:"

                # Поиск чисел в скобках в конце сообщения
                match = re.search(r"\((0(?:\.\d+)?|1(?:\.0)?)(?:\s*,\s*(\d{1,2}|50))?\)\s*$", user_message)
                if match:
                    denoising_strength = float(match.group(1))  # Первое число — denoising_strength
                    cfg_scale = int(match.group(2)) if match.group(2) else 8  # Второе число — cfg_scale, если указано
                    user_prompt = user_message[8:match.start()].strip()  # Текст до скобок
                else:
                    denoising_strength = 0.5  # Значение по умолчанию
                    cfg_scale = 8  # Значение по умолчанию
                    user_prompt = user_message[8:].strip()  # Весь текст после "дорисуй:"

                # Перевод пользовательского промпта, если он на русском

                steps_match = re.search(r"\[(\d+)\]$", user_prompt)
                if steps_match:
                    steps = int(steps_match.group(1))  # Извлекаем значение steps
                    user_prompt = re.sub(r"\[\d+\]$", "", user_prompt).strip()  # Удаляем [steps] из запроса
                else:
                    steps = 30  # Значение по умолчанию
                user_prompt = await translate_to_english_if_needed(user_prompt, user_id)
                ...
                job_id, payload  = await prodia_paint(prompt, image_base64, new_width, new_height, denoising_strength, cfg_scale, user_id, seed, steps)
                context.user_data['last_image_params'] = {
                    "prompt": prompt,
                    "width": width,
                    "height": height,
                ...
                if job_id:
                    ...
                    if image_url:
                        ...
                        escaped_caption = escape_gpt_markdown_v2(caption)                          
                        await update.message.reply_photo(photo=image_url, caption=escaped_caption, reply_markup=generated_inpaint_image_buttons, parse_mode="MarkdownV2")
                    else:
                        await waiting_message.edit_text("Не удалось получить изображение. Попробуйте снова.")
                else:
                    await waiting_message.edit_text("Произошла ошибка при отправке запроса. Попробуйте снова.")
...
async def retry_image_generat(update: Update, context: CallbackContext):


    # Повторяем генерацию изображения
    query = update.callback_query
    await query.answer()
    generated_image_buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("‼️Перезапуск бота‼️", callback_data='restart')],
    ...
    if not last_params:
        await query.message.reply_text("Не найдены параметры последней генерации.")
        return
    ...
    await query.message.reply_text("Создаём вариант изображения...")
    job_id, payload = await def_prodia(
        prompt=last_params["prompt"],
        width=last_params["width"],
        height=last_params["height"],
        cfg_scale = last_params['cfg_scale'],
        user_id=update.effective_user.id,
        seed=seed,
        steps = last_params['steps']
    )
    ...
    if job_id:
        ...
        await query.message.reply_text("Произошла ошибка при отправке запроса. Попробуйте снова.")



async def handle_generate_img2img_variants(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    await query.answer()
    generated_img2img_buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("‼️Перезапуск бота‼️", callback_data='restart')],
    ...
    last_params = context.user_data.get("last_image_params")
    if not last_params:
        await update.message.reply_text("Не найдены параметры последней генерации.")
        return
    # Функция для обновления denoising_strength
    def update_denoising_strength(value):
        delta = random.uniform(-0.1, 0.1)
        delta = round(delta, 2)  # Минимальный шаг изменений 0.02
        new_value = max(0.4, min(0.95, value + delta))  # Ограничение диапазона 0.4 - 0.95
        return new_value
    
    # Функция для обновления cfg_scale
    def update_cfg_scale(value):
        delta = random.randint(-2, 2)
        new_value = max(3, min(30, value + delta))  # Ограничение диапазона 3 - 30
        return new_value
    
    # Функция для обновления steps
    def update_steps(value):
        delta = random.randint(-3, 3)
        new_value = max(13, min(40, value + delta))  # Ограничение диапазона 13 - 40
        return new_value
    
    # Получаем и обновляем параметры
    denoising_strength = update_denoising_strength(last_params.get("settings", {}).get("denoising_strength", 0.5))
    cfg_scale = update_cfg_scale(last_params.get("settings", {}).get("cfg_scale", 7))
    steps = update_steps(last_params.get("settings", {}).get("steps", 25))
    seed = last_params.get("settings", {}).get("seed", random.randint(1, 4000000000))  # Генерируем случайный seed, если не найден

    # Извлекаем другие параметры
    prompt = last_params.get("prompt", "No prompt provided")
    img_base64 = last_params.get("imageData")
    mask_base64 = last_params.get("maskData")
    new_width = last_params.get("width")
    new_height = last_params.get("height")
    inpainting_fill = last_params.get("settings", {}).get("inpainting_fill")
    mask_blur = last_params.get("settings", {}).get("mask_blur")
    # Сообщение о начале генерации
    await query.message.reply_text("Создаём варианты изображения...")

    job_id, payload = await prodia_fill(
        prompt=prompt,
        imageData=img_base64,
        maskData=mask_base64,
        width=new_width,
        height=new_height,
        settings={
            "denoising_strength": denoising_strength,
    ...
    # Проверяем статус и отправляем изображение
    if job_id:
        image_url = await check_prodia_status(job_id)
        if image_url:
            # Формируем информацию о модели и параметрах
            style_preset = payload.get("style_preset", "Пресет не задан")
            model_name_map = {
                "dreamshaperXL10_alpha2.safetensors [c8afe2ef]": "DreamshaperXL",
                "sd_xl_base_1.0.safetensors [be9edd61]": "Stable Diffusion XL",
            ...
            escaped_caption = escape_gpt_markdown_v2(caption)
            await query.message.reply_photo(photo=image_url, caption=escape_gpt_markdown_v2(caption), reply_markup=generated_img2img_buttons, parse_mode="MarkdownV2")
        else:
            await query.message.reply_text("Не удалось получить изображение. Попробуйте снова.")
    else:
        await query.message.reply_text("Произошла ошибка при отправке запроса. Попробуйте снова.")
...
async def translate_to_english_if_needed(input_prompt, user_id):
    """
    Проверяет язык текста и переводит его на английский, если он не на английском.
    
    :param input_prompt: Текст запроса, может быть как `prompt`, так и `user_prompt`.
    :param user_id: ID пользователя.
    :return: Переведённый текст или оригинал, если текст уже на английском.
    """
    try:
        # Определяем язык текста
        detected_language = detect(input_prompt)
        logging.info(f"Определённый язык: {detected_language}")

        if detected_language != 'en':  # Если текст не на английском, переводим
            translated_prompt = await translate_to_english(user_id, input_prompt)
            logging.info(f"Передаваемый текст: {input_prompt}")
            return translated_prompt
        else:
            return input_prompt  # Если текст на английском, возвращаем как есть

    except Exception as e:
        logging.error(f"Ошибка при определении языка или переводе: {e}")
        return input_prompt  # Возвращаем оригинал в случае ошибки



async def handle_generate_inpaint_variants(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    await query.answer()
    ...
    # Проверяем параметры последнего изображения
    last_params = context.user_data.get("last_image_params")
    if not last_params:
        await update.message.reply_text("Не найдены параметры последней генерации.")
        return
    ...
    def update_denoising_strength(value):
        delta = random.uniform(-0.1, 0.1)
        delta = round(delta, 2)  # Минимальный шаг изменений 0.02
        new_value = max(0.35, min(0.65, value + delta))  # Ограничение диапазона 0.4 - 0.95
        return new_value
    ...
    cfg_scale = update_cfg_scale(last_params.get("settings", {}).get("cfg_scale", 7))
    steps = update_steps(last_params.get("settings", {}).get("steps", 25))
    seed = last_params["seed"]
    prompt = last_params["prompt"]

    # Конвертируем изображение в base64
    img_base64 = last_params.get("image_base64")

    # Отправляем сообщение "Создаём варианты..."
    await query.message.reply_text("Создаём вариант дорисовки...")

    # Рассчитываем новые размеры
    new_width = last_params["new_width"]
    new_height = last_params["new_height"]
    # Отправляем запрос на дорисовку
    job_id, payload = await prodia_paint(
        prompt=prompt,
        img_base64=img_base64,  # Используем правильное имя аргумента
        width=new_width,
        height=new_height,
        denoising_strength=denoising_strength,
        cfg_scale=cfg_scale,
    ...
    if job_id:
        ...
        if image_url:
            ...
            model_name = model_name_map.get(payload["model"], payload["model"])
            caption = (
                f"Вариант дорисовки изображения ID{payload['seed']} со следующими параметрами:\n"                    
                f"      Модель: `\"{model_name}\",`\n"
                f"      Стиль: `\"{style_preset}\",`\n"                            
                f"      cfg_scale: `{cfg_scale},`\n"
                f"      Денойзинг: `{denoising_strength},`\n"
                ...
                f"      Seed: `{payload['seed']},`\n"
                f"      Steps: `{payload['steps']}.`\n\n"                         
                f"Запрос: `Дорисуй: {prompt} ({denoising_strength}, {cfg_scale})`\n\n"

            )
            escaped_caption = escape_gpt_markdown_v2(caption)

            await query.message.reply_photo(photo=image_url, caption=escaped_caption, reply_markup=generated_inpaint_image_buttons, parse_mode="MarkdownV2")
        else:
            await query.message.reply_text("Не удалось получить изображение. Попробуйте снова.")
    else:
        await query.message.reply_text("Произошла ошибка при отправке запроса. Попробуйте снова.")
...
async def handle_generate_variants(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    await query.answer()
    generated_image_buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("‼️Перезапуск бота‼️", callback_data='restart')],
...
async def extract_prompt(caption: str, user_id: int) -> str:
    """
    Извлекает текст после "замени:" (в любом регистре) и до "(config:", 
    игнорируя переносы строк, и переводит его на английский, если это необходимо.
    
    :param caption: Строка с описанием.
    :param user_id: ID пользователя.
    :return: Извлечённый и, если нужно, переведённый текст.
    """
    # Регулярное выражение для поиска текста
    match = re.search(r"(?i)^замени:\s*(.*?)(\s*\(config:.*)?$", caption, re.DOTALL)
    if match:
        # Извлекаем текст из первой группы (до "(config:")
        prompt = match.group(1).strip()
        # Переводим текст на английский, если это необходимо
        prompt = await translate_to_english_if_needed(prompt, user_id)
        return prompt
...
def parse_settings(caption: str, default_settings: dict) -> dict:
    ...
    Поддерживает форматы "ключ = значение" и "ключ: значение".
    Учитывает переносы строк, пробелы и регистр.
    """
    # Убираем всё до и включая 'config:', чтобы избежать проблем с лишними текстами
    caption = caption.split("config:")[-1]

    # Регулярное выражение для поиска пар ключ-значение
    settings_pattern = re.findall(r"(?i)(\w+)\s*(?:=|:)\s*([\s\S]+?)(?=\s*(?:,|$|\)|\n)|\s*$)", caption)

    # Копируем настройки по умолчанию
    parsed_settings = default_settings.copy()
    logging.info(f"Полученные параметры: {caption}")
    logging.info(f"Найденные пары ключ-значение: {settings_pattern}")
    ...
    for key, value in settings_pattern:
        key = key.lower().strip()  # Приводим ключ к нижнему регистру
        value = value.strip()  # Убираем лишние пробелы и переносы строк
        if key in parsed_settings:
            try:
                if isinstance(default_settings[key], bool):
                    parsed_settings[key] = to_bool(value)
                ...
                logging.error(f"Ошибка преобразования ключа {key} со значением {value}: {e}")
    logging.info(f"Преобразованные параметры: {parsed_settings}")
...
async def prodia_fill(
    ...
    payload = {
        ...
        "prompt": prompt,        
        "negative_prompt": "(easynegative:1.5), disfigured, poorly drawn, low quality, harsh shadows, overly stylized, unintended textures, text errors, extra limbs, blurry",
        "steps": settings.get("steps", 23),
        "cfg_scale": settings.get("cfg_scale", 12),
        "seed": settings.get("seed", 12),
        "upscale": False,
...
async def prodia_paint(prompt: str, img_base64: str, width: int, height: int, denoising_strength: float, cfg_scale: int, user_id: int, seed: int, steps: int) -> str:
    ...
    payload = {
        ...
        "negative_prompt": "(easynegative:1.5), disfigured, poorly drawn, cartoonish, low quality, harsh shadows, overly stylized, unintended textures, text errors, extra limbs, blurry",
        "steps": steps,
...
def generate_preset_buttons():
    ...
    buttons.append([InlineKeyboardButton("❌Отмена❌", callback_data="preset_cen")])
    return InlineKeyboardMarkup(buttons)
...
async def handle_preset_selection(update: Update, context: CallbackContext):
    ...
    elif query.data == "preset_cen":
        # Удаляем сообщение
        await query.message.delete()
        await query.answer("Вы отменили выбор.")
...
async def def_prodia(prompt: str, width: int, height: int, cfg_scale: int, user_id: int, seed: int, steps: int) -> str:
    ...
    payload = {
        ...
        "steps": steps,
        "cfg_scale": cfg_scale,
        "seed": seed,  # Используем переданный seed
        "sampler": "DPM++ 2M Karras",
        "upscale": False,
        "width": width,
        "height": height
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(handle_generate_variants, pattern="^generate_variants$"))
    application.add_handler(CallbackQueryHandler(handle_generate_inpaint_variants, pattern="^generate_inpaint_variants$"))
    application.add_handler(CallbackQueryHandler(handle_generate_img2img_variants, pattern="^generate_img2img_variants$"))
    application.add_handler(CallbackQueryHandler(handle_preset_selection, pattern="^preset_cen$"))
    application.add_handler(CallbackQueryHandler(retry_image_generat, pattern="^retry_image_generat$")) 
