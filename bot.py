        ...
        try:
            ...
            if response_json.get('ok'):
                ...
                if image_count > 1:
                    ...
                    media_group_data = []  # Для хранения данных о медиагруппе (file_id, caption)
                        ...
                        for idx, item in enumerate(group):
                            if item['type'] == 'image':
                                if idx == 0:  # Добавляем заголовок только для первого изображения
                                    ...
                                    media_group_data.append({
                                        "file_id": item['url'],
                                        "caption": None  # Для остальных изображений нет подписи
                                    })
                
                        # Попытка отправить медиа группу
                        try:
                            await update.message.reply_media_group(media_group)
                        except Exception as e:
                            await update.message.reply_text(f'🚫Ошибка при отправке медиа: {str(e)}')
                            return
                
                    # Сохраняем данные медиагруппы в глобальный словарь
                    media_group_storage[user_id] = media_group_data
                
                    # Используем message_with_link для отправки сообщения с кнопкой
                    ...
                    if single_image:
                        ...
                        # Сохраняем данные о единственном изображении в хранилище
                    ...
                    message_with_link = f'{author_line}\n<a href="{article_url}">Оригинал</a>'
                    await update.message.reply_text(message_with_link, parse_mode='HTML', reply_markup=create_publish_button(user_id))
