...
async def generate_gemini_inline_response(query: str) -> str:
    ...
    system_instruction = (
        "Ты умная и лаконичная нейросеть. Отвечай кратко, по сути запроса и по существу, избегая вводных фраз и лишних размышлений. Длинные ответы давай только когда это действительно требуется"
    )
